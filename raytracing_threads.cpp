...
void threadFunc(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int startY, int endY, int numOfSamples) {
...
int main(int argc, char **argv) {
    ...
    int block_size = viewPlaneResolutionY/numberOfThreads;
  ...
  // TODO: make each thread to compute different part of the image.
    ...
    for (int i = 0; i < numberOfThreads; i++) {
        thread thr(threadFunc, ref(scene), ref(viewPlane), ref(image), 
            viewPlaneResolutionX, i*block_size, (i+1)*block_size, numOfSamples);
